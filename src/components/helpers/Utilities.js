...
/**
 * Utilities class containing a variety of methods
...
export default class Utilities {
  ...
  /**
   * Generates all numbers to be called for this game
   * We do this so we can cut down on processing for constantly generating new calls
   *
   * @return  {Array}  Array of all numbers in order for calling in this game
   */
  static generateBingoCalls = () => {
    // Obtain all randomized balls that will be called on our game
    // this way we do not have to call a new random ball each time
    let byteArray = new Uint8Array(1);
    let allCalls = [];

    while (allCalls.length < 75) {
      let randomCall = window.crypto.getRandomValues(byteArray)[0];
      if (
        randomCall > 0 &&
        randomCall <= 75 &&
        !allCalls.includes(randomCall)
      ) {
        allCalls.push(randomCall);
      }
    }
    return allCalls;
  };
